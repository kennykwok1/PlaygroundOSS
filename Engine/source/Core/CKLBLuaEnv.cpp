...
void
CKLBLuaEnv::addSearchPath(const char *path)
{
    lua_getglobal(m_L, "package");                                  /* L: package */
    lua_getfield(m_L, -1, "path");                /* get package.path, L: package path */
    const char *cur_path =  lua_tostring(m_L, -1);
    lua_pushfstring(m_L, "%s;%s/?.lua", cur_path, path);            /* L: package path newpath */
    lua_setfield(m_L, -3, "path");          /* package.path = newpath, L: package path */
    lua_pop(m_L, 2);
}
