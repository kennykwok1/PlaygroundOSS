...
CKLBLuaScript::create(const char *bootScriptURL)
{
    ...
    //add search scripts path to package search path
    const char * bootFullPath = CPFInterface::getInstance().platform().getFullPath(bootScriptURL);
    std::string path = bootFullPath;
    size_t pos;
    ...
    if (p != path.npos)
    {
        const std::string dir = path.substr(0, p) + "/scripts";
        CKLBLuaEnv::getInstance().addSearchPath(dir.c_str());
        p = dir.find_last_of("/\\");
        if (p != dir.npos)
        {
            std::string dirOther = dir.substr(0, p) + "/scripts";
            CKLBLuaEnv::getInstance().addSearchPath(dirOther.c_str());
        }
    }
    //
